...
OQS_SIG *OQS_SIG_ml_dsa_44_new(void) {
	...
	sig->keypair_from_seed = OQS_SIG_ml_dsa_44_keypair_from_seed;
...
extern int pqcrystals_ml_dsa_44_ref_keypair_from_seed(uint8_t *pk, uint8_t *sk, const uint8_t *seed, size_t seed_len);
...
extern int pqcrystals_ml_dsa_44_avx2_keypair_from_seed(uint8_t *pk, uint8_t *sk, const uint8_t *seed, size_t seed_len);
...
OQS_API OQS_STATUS OQS_SIG_ml_dsa_44_keypair_from_seed(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed, size_t seed_len) {
#if defined(OQS_ENABLE_SIG_ml_dsa_44_avx2)
#if defined(OQS_DIST_BUILD)
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) pqcrystals_ml_dsa_44_avx2_keypair_from_seed(public_key, secret_key, seed, seed_len);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_ml_dsa_44_ref_keypair_from_seed(public_key, secret_key, seed, seed_len);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) pqcrystals_ml_dsa_44_ref_keypair_from_seed(public_key, secret_key, seed, seed_len);
