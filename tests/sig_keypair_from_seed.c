...
static OQS_STATUS test_keypair_from_seed_twice(const char *method_name) {
	...
	for (uint8_t i = 0; i < ENTROPY_INPUT_LENGTH; i++) {
		entropy_input[i] = i;
	}
...
static OQS_STATUS test_other_algorithms_return_error(void) {
	...
	uint8_t dummy_seed[32] = {0};  // Dummy seed
	...
	for (size_t i = 0; test_algorithms[i] != NULL; i++) {
		...
		OQS_STATUS rc = OQS_SIG_keypair_from_seed(sig, pk, sk, dummy_seed, 32);
...
static OQS_STATUS test_clean_vs_avx2_keypair_consistency(const char *method_name) {
	...
	rc_clean = PQCLEAN_FALCON512_CLEAN_crypto_sign_keypair_from_seed(pk_clean, sk_clean, seed, SEED_LENGTH);
	if (rc_clean != 0) {
	...
	rc_avx2 = PQCLEAN_FALCON512_AVX2_crypto_sign_keypair_from_seed(pk_avx2, sk_avx2, seed, SEED_LENGTH);
	if (rc_avx2 != 0) {
...
int main(void) {
	...
	OQS_STATUS rc2 = test_clean_vs_avx2_keypair_consistency("Falcon-512");
	if (rc2 != OQS_SUCCESS) {
		overall_result = OQS_ERROR;
